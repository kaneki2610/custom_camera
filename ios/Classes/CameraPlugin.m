...
- (void)captureOutput:(AVCapturePhotoOutput *)output
  ...
  bool success = [UIImageJPEGRepresentation(imageSave, 1.0) writeToFile:_path atomically:YES];
  if (!success) {
...
- (void)zoom:(NSUInteger)step{
  _zoom += step;
    if(_zoom > _captureDevice.activeFormat.videoMaxZoomFactor){
        _zoom = _captureDevice.activeFormat.videoMaxZoomFactor;
    }
